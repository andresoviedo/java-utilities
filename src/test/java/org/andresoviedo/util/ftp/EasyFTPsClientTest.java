...
public class EasyFTPsClientTest {
	...
	public void setUp() throws ParseException, IOException, FtpException {
		constantDate = new SimpleDateFormat("yyyy/MM/dd HH:mm").parse(CONSTANT_DATE_STRING);
		constantDate2 = new SimpleDateFormat("yyyy/MM/dd HH:mm").parse(CONSTANT_DATE2_STRING);

		// get free port
		int freePort = freePort();

		// get temporary home directory
		File homeDirectory = new File("target" + File.separator + "generated-test-sources/sftp");
		...
		File confirmationDir = new File(bankFolder, EntityConstants.CONFIRMATION_FOLDER);
		...
		ssl.setKeystoreFile(new File("src/test/resources/org/andresoviedo/util/ftp/sftp_keystore.jks"));
		...
		userManagerFactory.setFile(new File("src/test/resources/org/andresoviedo/util/ftp/sftp_users.properties"));
		UserManager um = userManagerFactory.createUserManager();
		// BaseUser user = new BaseUser();
		// user.setName("dummy");
		// user.setPassword("ymmud");
		// user.setHomeDirectory(homeDirectory.getPath());
		// user.setAuthorities(Collections.singletonList((Authority) new WritePermission()));
		// um.save(user);
	...
	public void testPut() {
		sut.put(generateFilename(EntityConstants.FileType.REQUEST, constantDate),
				getClass().getResourceAsStream("/org/andresoviedo/util/serialization/api2/R_EEEEAAAAMMDDHHMM.txt"));
	}

	@Test
	public void testGet() {
		File outputFolder = new File("target" + File.separator + "generated-test-sources" + File.separator + "output");
		outputFolder.mkdirs();
