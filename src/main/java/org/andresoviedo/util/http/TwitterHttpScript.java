...
public class TwitterHttpScript {
	...
	private void initYahooRemoveAndAddContactScript() {
		...
		// Step 1: Get a Request Token (get_request_token)
		{
			...
			TreeNode<Step> node = yahooHttpScript
					.addRequest(yahooHttpScript.new HttpRequest("get_request_token",
							"https://api.login.yahoo.com/oauth/v2/get_request_token", 443, HttpScript.Method.POST,
							params, null));
			node.addChildren(yahooHttpScript.new RegexExtractor("oauth_token", "oauth_token", "oauth_token=([^\\&]+)",
					ResponseField.BODY));
			node.addChildren(yahooHttpScript.new RegexExtractor("oauth_token_secret", "oauth_token_secret",
					"oauth_token_secret=([^\\&]+)", ResponseField.BODY));
		...
		// Step 2: Get User Authorization (request_auth)
		{
			// Go login page to get parameters
			{
				...
				TreeNode<Step> node = yahooHttpScript
						.addRequest(yahooHttpScript.new HttpRequest("request_auth",
								"https://api.login.yahoo.com/oauth/v2/request_auth", 443, HttpScript.Method.POST,
								params, null));
				...
				node.addChildren(yahooHttpScript.new RegexExtractor("_ts", "_ts",
						"<input name=\"_ts\" .*?value=\"([^\"]*)\">", ResponseField.BODY, Lookup.SUBSAMPLES, 0));
				node.addChildren(yahooHttpScript.new RegexExtractor("_uuid", "_uuid",
						"<input .*?name=\"_uuid\" .*?value=\"([^\"]*)\">", ResponseField.BODY, Lookup.SUBSAMPLES, 0));
				node.addChildren(yahooHttpScript.new RegexExtractor("_seqid", "_seqid",
						"<input .*?name=\"_seqid\" .*?value=\"([^\"]*)\">", ResponseField.BODY, Lookup.SUBSAMPLES, 0));
				node.addChildren(yahooHttpScript.new RegexExtractor("otp_channel", "otp_channel",
						"<input .*?name=\"otp_channel\" .*?value=\"([^\"]*)\">", ResponseField.BODY, Lookup.SUBSAMPLES,
						0));
				node.addChildren(yahooHttpScript.new RegexExtractor("_crumb", "_crumb",
						"<input .*?name=\"_crumb\" .*?value=\"([^\"]*)\">", ResponseField.BODY, Lookup.SUBSAMPLES, 0));
			...
			// Do login
			{
				...
				TreeNode<Step> node = yahooHttpScript.addRequest(yahooHttpScript.new HttpRequest("login",
						"${yahoo_login_url}", 443, HttpScript.Method.POST, params, null));
				node.addChildren(yahooHttpScript.new RegexExtractor("url", "_redirect_url", "\"url\":\"([^\"]+)\"",
			...
			// Login redirects us... get crumb
			{
				...
				node.addChildren(yahooHttpScript.new RegexExtractor("crumb", "crumb",
						"<input .*?name=\"crumb\" .*?value=\"([^\"]*)\">", ResponseField.BODY, Lookup.SUBSAMPLES, 0));
			}
			// Yahoo asks user to grant permission
			{
				...
				node.addChildren(yahooHttpScript.new RegexExtractor("oauth_verifier", "oauth_verifier",
						"oauth_verifier=([^&]+)\\&?", ResponseField.HEADERS, Lookup.MAIN, 0));
			}
		}
		...
		// Step 3: Exchange the Request Token and OAuth Verifier for an Access Token (get_token)
		{
			...
			TreeNode<Step> node = yahooHttpScript.addRequest(yahooHttpScript.new HttpRequest("get_token",
					"https://api.login.yahoo.com/oauth/v2/get_token", 443, HttpScript.Method.GET, params, null)
					.setFollowRedirects(false));
			node.addChildren(yahooHttpScript.new RegexExtractor("oauth_token", "oauth_token", "oauth_token=(.+?)\\&",
					ResponseField.BODY, Lookup.MAIN, 0));
			node.addChildren(yahooHttpScript.new RegexExtractor("oauth_token_secret", "oauth_token_secret",
					"oauth_token_secret=(.+?)\\&", ResponseField.BODY, Lookup.MAIN, 0));
		...
		// Step 5: Get all yahoo contacts ir order to delete them
		{
			TreeNode<Step> node = yahooHttpScript.addRequest(yahooHttpScript.new HttpRequest("get-all-contacts",
					"https://social.yahooapis.com/v1/user/${xoauth_yahoo_guid}/contacts", 443, HttpScript.Method.GET,
					null, authHeader));
			...
			{
				TreeNode<Step> forNode = node.addChildren(yahooHttpScript.new ForEach("for-each-contact",
						"contactsuris", "contacturi"));
		...
		// Step 7: Add contact
		{
			...
			TreeNode<Step> node = yahooHttpScript.addRequest(yahooHttpScript.new HttpRequest("get_token",
					"https://social.yahooapis.com/v1/user/${xoauth_yahoo_guid}/contacts", 443, HttpScript.Method.PUT,
					json_yahooAddContact, authHeader));
			node.addChildren(yahooHttpScript.new RegexExtractor("sucess", "sucess", "(\"response\":\"success\")",
					ResponseField.BODY, Lookup.MAIN, 0));
		}
	}
	...
	private void initTwitterImportContactsAndGetName() {
		...
		}
		{
			...
			// login on twitter
			{
				...
				twitterHttpScript.addRequest(twitterHttpScript.new HttpRequest("twitter_login",
						"https://twitter.com/sessions", 443, HttpScript.Method.POST, params, null));
			}
			...
			// wipe contacts!
			{
				twitterHttpScript.addRequest(twitterHttpScript.new HttpRequest("wipe-contacts",
						"https://twitter.com/settings/contacts/wipe_addressbook", 443, HttpScript.Method.POST,
						Collections.singletonMap("authenticity_token", "${authenticity_token}"), null));
		...
		// import contacts from yahoo
		{
			...
			TreeNode<Step> node = twitterHttpScript.addRequest(twitterHttpScript.new HttpRequest(
					"twitter_oauth_launch", "https://twitter.com/invitations/oauth_launch", 443, HttpScript.Method.GET,
					params, null));
			node.addChildren(twitterHttpScript.new RegexExtractor("regex_20", "twitter_oauth_token",
					"<a href=\"https://api\\.login\\.yahoo\\.com/oauth/v2/request_auth\\?oauth_token=(.+?)\">",
					ResponseField.BODY, Lookup.MAIN, 0));
			node.addChildren(twitterHttpScript.new RegexExtractor("_ts", "_ts",
					"<input .*?name=\"_ts\".*? value=\"(.+?)\">", ResponseField.BODY, Lookup.SUBSAMPLES, 0));
			node.addChildren(twitterHttpScript.new RegexExtractor("_uuid", "_uuid",
					"<input .*?name=\"_uuid\".*? value=\"(.+?)\">", ResponseField.BODY, Lookup.SUBSAMPLES, 0));
			node.addChildren(twitterHttpScript.new RegexExtractor("_seqid", "_seqid",
					"<input .*?name=\"_seqid\".*? value=\"(.+?)\">", ResponseField.BODY, Lookup.SUBSAMPLES, 0));
			...
			node.addChildren(twitterHttpScript.new RegexExtractor("_crumb", "_crumb",
					"<input .*?name=\"_crumb\".*? value=\"(.+?)\">", ResponseField.BODY, Lookup.SUBSAMPLES, 0));
			node.addChildren(twitterHttpScript.new RegexExtractor("yahoo_login_url", "yahoo_login_url",
					"location: (.+)", ResponseField.HEADERS, Lookup.SUBSAMPLES, 0));
		}
		...
		// Login to yahoo
		{
			...
			TreeNode<Step> node = twitterHttpScript.addRequest(twitterHttpScript.new HttpRequest("yahoo_login",
					"${yahoo_login_url}", 443, HttpScript.Method.POST, params, null));
			...
			// ResponseField.BODY, Lookup.MAIN, 0));
			node.addChildren(twitterHttpScript.new RegexExtractor("_redirect_url", "_redirect_url",
					"\"url\":\"([^\"]+)\"", ResponseField.BODY, Lookup.MAIN));
		}
		...
		// login redirect
		{
			...
			node.addChildren(twitterHttpScript.new RegexExtractor("crumb", "crumb", "location: .*crumb=([^\\&]+)",
		...
		// wait twitter to import contacts...
		{
			...
			node.addChildren(twitterHttpScript.new RegexExtractor("import_status", "import_status",
					"\"done\":(true|false)", ResponseField.BODY, Lookup.MAIN, 0));
			node.addChildren(twitterHttpScript.new RegexExtractor("import-response", "matched_count",
					"\\{\"done\":false\\}|\\{\"done\":true,\"matched_count\":(.),\"unmatched_count\":.\\}",
					ResponseField.BODY, Lookup.MAIN, 0));
		}
		// get fullname :)
		{
	...
	public TwitterResult execute(String email, String yahooUser, String yahooPass, String twitterUser,
			String twitterPass, String yahooConsumerKey, String yahooConsumerSecret) {
		...
		try {
			...
			if (twitterEx instanceof Exception) {
				...
				return new TwitterResult(Status.ERROR, null, "Twitter-while exception: "
						+ ((Exception) twitterEx).getMessage() + ": " + twitterResult);
			}
			...
			if (!"1".equals(matchedCount)) {
				return new TwitterResult(Status.ERROR, null, "Unexpected match_count '" + matchedCount + "': "
						+ twitterResult);
			}
