 ...
 * <ul>
 * <li>Task are identified by id. Tasks can depend on the finalization of
 * dependant tasks.</li>
...
public class DependantTasksExecutor extends ThreadPoolExecutor {
	...
	private static final Log logger = LogFactory
			.getLog(DependantTasksExecutor.class);
	/**
	...
	public <T> Future<T> submit(final Callable<T> task, final String taskId,
			final String[] dependantTasksId) {
		// TODO: tasks should not dependent on inexistent tasks neither
		// dependeing on itself

		Callable<T> wrappedTask = new DependantCallable<T>(taskId, task,
				dependantTasksId);
	...
	public void submit(final Runnable task, final String taskId,
			final String[] dependantTasksId) {

		Runnable wrappedTask = new Runnable() {
			@Override
			public void run() {
				Callable<Void> c = new DependantCallable<Void>(taskId,
						new Callable<Void>() {
							public Void call() {
								task.run();
								return null;
	...
	class DependantCallable<T> implements Callable<T> {
		...
		public DependantCallable(String taskId, Callable<T> originalTask,
				String... dependantTasksId) {
			super();
			this.taskId = taskId;
		...
		public T call() throws Exception {
			...
			if (dependantTasksId != null && dependantTasksId.length > 0) {
				...
				synchronized (tasks) {
					for (int i = 0; i < dependantTasksId.length;) {
						logger.trace("Waiting for dependant task number '"
								+ dependantTasksId[i] + "'...");
						Future<?> dependantTask = tasks
								.get(dependantTasksId[i]);
						if (!dependantTask.isDone()) {
							logger.trace("Dependant task with number '"
									+ dependantTasksId[i]
									+ "' has not yet finalized. Waiting...");
							i = 0;
							tasks.wait(1000);
						} else {
							try {
			...
			try {
				...
				logger.trace("Executing task '" + taskId
						+ "' after all it's dependencies has been satisfied...");
			...
			synchronized (tasks) {
				logger.trace("Task '" + taskId
						+ "' finalized. Notifying any dependant task...");
				tasks.notifyAll();
			}
	...
	public static void main(String[] args) throws InterruptedException {
		...
		executor.submit(new Callable<Integer>() {
			...
			public Integer call() throws Exception {
				...
				System.out.println("<< 22222 ["
						+ Arrays.toString((Object[]) DependantTasksExecutor.results
								.get()));
				Thread.sleep((long) ((Math.random() * 5000)));
				System.out.println(">> 22222");
				return 2;
