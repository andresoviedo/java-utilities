...
public final class IOHelper {
	...
	public static void copyAndFilter(File sourceFile, File targetFile, Dictionary<String, String> dictionary) throws IOException {

		if (sourceFile.isDirectory()) {
			targetFile.mkdirs();
			for (File file : sourceFile.listFiles()) {

				copyAndFilter(file, new File(targetFile, file.getName()), dictionary);
			}
			...
			if (targetFile.getParent() != null && targetFile.getParentFile().exists()) {
	...
	public static File filterFile(File sourceFile, String pattern, String replace) throws IOException {
		String content = FileUtils.readFileToString(sourceFile);
		File tempFile = File.createTempFile("arq-sdk-" + System.currentTimeMillis(), ".tmp");
		FileUtils.writeStringToFile(tempFile, content.replaceAll(pattern, Matcher.quoteReplacement(replace)));
		return tempFile;
	}

	public static File filterFile(File sourceFile, Dictionary<String, String> dictionary) throws IOException {
		String content = FileUtils.readFileToString(sourceFile);
		File tempFile = File.createTempFile("arq-sdk-" + System.currentTimeMillis(), ".tmp");
	...
	public static File copyResourceToTempFile(String resourcePath) throws IOException {
		...
		return copyResourceToTempFile(resourcePath, extension != -1 ? resourcePath.substring(extension) : null);
	}

	public static File copyResourceToTempFile(String resourcePath, String suffix) throws IOException {
		int resourceName = resourcePath.lastIndexOf("/");
		File tempFile = File.createTempFile("org.andresoviedo.util." + (resourceName != -1 ? resourcePath.substring(resourceName + 1) : "")
				+ ".", suffix);
		FileUtils.copyInputStreamToFile(IOHelper.class.getResourceAsStream(resourcePath), tempFile);
		return tempFile;
	}
}
