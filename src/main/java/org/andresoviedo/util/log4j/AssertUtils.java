...
public final class AssertUtils {
	...
	static class LogUtils {
		...
		public static void assertLogsRegExp(Appender mockAppender, String message, boolean exists, String... expectedMessagesRegExp) {
			// TODO: Implementar la activaci�n din�mica de logs (consola y
			// memoria)
			// if (!this.enabledLogs) {
			// throw new
			...
			// mensajes
			// con exists == false ning�n mensaje debe existir
			if (exists == true) {
				for (int i = 0; i < messagesExist.length; i++) {
					if (!messagesExist[i]) {
						fail("No se ha podido encontrar el log con la regular expressi�n '" + expectedMessagesRegExp[i] + "'");
					}
				}
			} else {
