...
public abstract class Messenger implements SocketSessionController {
	...
	/**
	 * The map holding <code>Persistence</code> objects, one for each registered
	 * service.
	 */
	...
	/**
	 * A static map with registered classes (needed to access some classes when
	 * using different class loaders).
	 */
	private static Map<String, Class<?>> registeredClasses = new Hashtable<String, Class<?>>();

	/**
	 * Maximum number of service parallel threads invoking the listeners
	 * processMessage method.
	 */
	private int maxParallelThreads = MessengerProperties
			.getMaxParallelThreads();

	/**
	 * This flag is set to true when all ServiceThreads should pause it's
	 * execution
	 */
	...
	/**
	 * A list of SessionMessages received by all SocketSessions ready to be
	 * processed by service threads
	 */
	 ...
	 *             if the message to be sent is not a valid message or an error
	 ...
	 *             occurs while sending the message.
	 */
	protected void send(Message message, boolean persist)
			throws MessengerException {
		...
		if (persist) {
			try {
				logger.fine("Adding message with id '" + message.getMessageId()
						+ "' to the underlying persistence mechanism...");
				persistence.add(message);
			} catch (PersistenceException e) {
				logger.severe("Error persisting message with id '"
		...
		if (!message.isNeedsAck()) {
			if (!persistence.delete(message)) {
				logger.warning("Message with id '" + message.getMessageId()
	 ...
	 *             if an error occurs while sending the message.
	 */
	protected void send(Message message, SocketSession session)
			throws MessengerException {
		...
		if (!session.containsProperty(SESSION_AUTHENTICATED_PROPERTY)) {
			throw new MessengerException(
					"Session is not authenticated. Cannot send message with id '"
							+ message.getMessageId() + "'.");
		}
		try {
			session.send(message);
		} catch (SocketSessionException e) {
			logger.warning("Exception caught while sending the message: "
					+ e.getMessage());
			throw new MessengerException(
					"Exception caught while sending the message.", e);
		}
	}

	/**
	 * Sends a message and waits for a response to be received. May return
	 * <code>null</code> if the message cannot be sent.
	 * 
	 * @param message
	 *            the message to send.
	 ...
	 */
	public Message sendAndReceive(Message message) throws MessengerException,
	 ...
	 *            the message to send.
	 * @param timeout
	 *            the maximum number of milliseconds to wait until a message is
	 *            received. Zero means infinite timeout.
	 * @return the response message.
	 ...
	 *             if the current thread is interrupted waiting for a response.
	 */
	public Message sendAndReceive(Message message, long timeout)
	 ...
	 *             if the current thread is interrupted waiting for a response.
	 */
	protected abstract Message sendAndReceiveImpl(Message message, long timeout)
			throws MessengerException, InterruptedException;
	 ...
	 *             if the current thread is interrupted waiting for a response.
	 */
	protected Message sendAndReceive(Message message, SocketSession session,
			long timeout) throws MessengerException, InterruptedException {
		// Check whether this session is authenticated or not.
		if (!session.containsProperty(SESSION_AUTHENTICATED_PROPERTY)) {
			throw new MessengerException(
					"Session is not authenticated. Cannot send message with id '"
	 ...
	 * @throws MessengerException
	 *             if the message is <code>null</code> or considered to be
	 *             invalid.
	 */
	public void validate(Message message) throws MessengerException {
		...
		if ((message.getClientId() == null)
				|| (message.getClientId().length() == 0)) {
			throw new MessengerException("Invalid message: no client id found.");
		}
		if ((message.getMessageId() == null)
				|| (message.getMessageId().length() == 0)) {
			throw new MessengerException(
					"Invalid message: no message id found.");
		}
		if ((message.getServiceId() == null)
				|| (message.getServiceId().length() == 0)) {
			throw new MessengerException(
					"Invalid message: no service id found.");
		}
		if ((message.getTargetServiceId() == null)
				|| (message.getTargetServiceId().length() == 0)) {
			throw new MessengerException(
					"Invalid message: no target service id found.");
		}
	}
	...
	public void addSecondaryService(String primaryServiceId,
		...
		try {
			listenerList.addSecondaryService(primaryServiceId,
					secondaryServiceId);
			...
			for (Iterator<?> it = persistence.iterator(false); it.hasNext();) {
				...
				if (message != null) {
					...
					if (!persistence.delete(message)) {
						logger.warning("Message with id '"
								+ message.getMessageId()
								+ "' could not be deleted.");
					}
				}
			...
			logger.severe("Exception caught while forwarding the message: "
					+ e.getMessage());
			logger.throwing(getClass().getName(), "addSecondaryService",
					e.getCause());
		}
	}

	/**
	 * Associates the specified listener to a service id. This method does
	 * nothing if either <code>serviceId</code> or <code>l</code> are
	...
	public void setServiceListener(String serviceId, ServiceListener l) {
		...
		try {
			...
			logger.severe("Exception caught while forwarding the message: "
					+ e.getMessage());
			logger.throwing(getClass().getName(), "setServiceListener",
					e.getCause());
		}
	}

	/**
	 * Removes the listener associated to the specified service id. This method
	 * does nothing if <code>serviceId</code> is null.
	 * 
	 * @param serviceId
	 *            the service id.
	 */
		...
		if (command instanceof SignalingCommand) {
			...
			if (smsgsList == null) {
				synchronized (serviceMsgs) {
					...
					if (smsgsList == null) {
						List<ServiceThread> serviceThreadsList = new Vector<ServiceThread>(
								maxParallelThreads);
						serviceMsgs.put(targetServiceId,
								smsgsList = new Vector<SessionMessage>());
						serviceThreads.put(targetServiceId, serviceThreadsList);
						for (int i = 0; i < maxParallelThreads; i++) {
							ServiceThread st = new ServiceThread(smsgsList,
									targetServiceId + "_" + i);
	...
	public void messageReceived(SocketSession session, Message message) {
		...
		try {
			if (message.getCorrelationMessageId() == null) {
				...
				MessageLock lock = (MessageLock) locks.get(message
						.getCorrelationMessageId());
				if (lock != null) {
					synchronized (lock) {
						lock.result = message;
						lock.notify();
			...
			logger.info("Exception caught while forwarding the message: "
					+ e.getMessage());
			// Persist the message.
			try {
				addMessageToServicePersistence(message,
						message.getTargetServiceId());
			} catch (PersistenceException e1) {
				logger.severe("Exception caught while persisting the message: "
						+ e1.getMessage());
		...
		if (sendAck) {
			logger.fine("Message received with id '" + message.getMessageId()
					+ "', sending acknowledgement...");
			try {
				session.send(new MessageAck(message.getClientId(), message
						.getMessageId()));
				// Forward the message to secondary listeners if and only if
				// everything is OK.
				forwardMessageToSecondaryServices(message,
						message.getTargetServiceId());
			} catch (SocketSessionException e) {
	...
	private void signalingCommandReceived(SocketSession session,
		...
		if (command instanceof LoginResponse) {
			...
			if (response.getResult() == LoginResponse.LOGIN_OK) {
				...
				session.putProperty(SESSION_AUTHENTICATED_PROPERTY,
						Boolean.TRUE);
				...
				for (Iterator<?> it = persistence.iterator(clientId, false); it
					...
					message = (Message) it.next();
					if (message != null) {
						logger.fine("Sending persisted message with id '"
								+ message.getMessageId() + "'...");
						...
						try {
							...
							logger.warning("MessengerException caught: "
									+ e.getMessage());
						}
					}
				}
			...
			logger.fine("Message with id '" + ack.getMessageId()
					+ "' acknowledged, deleting it from persistence...");
			if (!persistence.delete(ack.getClientId(), ack.getMessageId())) {
				logger.warning("Message with id '" + ack.getMessageId()
						+ "' could not be deleted.");
			}
	 ...
	 * messageSent(ttm.platform.services.messaging.common.net.SocketSession ,
	 * ttm.platform.services.messaging.common.data.Message)
	 */
	...
	/*
	 * @see ttm.platform.services.messaging.common.net.SocketSessionController#
	 * sessionClosed(ttm.platform.services.messaging.common.net.SocketSession ,
	 * boolean)
	 */
	 ...
	 * @param forced
	 *            <code>true</code> if the session has been forced to be closed
	 *            using its <code>close()</code> method.
	 * @since 2.0.10
	 */
	public void fireSessionClosed(SocketSession session, boolean forced) {
		EventListener[] listeners = messengerListenerList
				.getListeners(MessengerListener.class);
	 ...
	 * If no listener is registered, the message is persisted.
	 * 
	 * @param message
	 ...
	 *            the service id.
	 * @throws MessengerException
	 *             if no service listener is available or an uncaught exception
	...
	protected void forwardMessage(Message message, String serviceId)
		...
		try {
			if (!listenerList.forwardMessage(message, serviceId)) {
				...
				throw new MessengerException("No service with id '" + serviceId
						+ "' is available.");
			}
		} catch (MessengerException e) {
			throw e;
			...
			// too.
			logger.severe("Exception caught while processing message: "
					+ e.getMessage());
	...
	/**
	 * Forwards the specified message to services associated to a primary
	 * service (the so-called secondary services).
	 * 
	 * @param message
	 *            the message.
	 * @param serviceId
	 ...
	 * @since 2.0.8
	 */
	protected void forwardMessageToSecondaryServices(Message message,
			String serviceId) {
		Set<?> secondaryServiceIds = listenerList
				.getSecondaryServiceIds(serviceId);
			...
			try {
				...
				if (listener != null) {
					...
					logger.throwing(getClass().getName(),
							"forwardMessageToSecondaryServices", e);
				}
				try {
	...
	/**
	 * @return the maximum number of parallel Reader threads that can run at
	 *         same time.
	 */
	...
	/**
	 * Adds the specified message to the persistence object associated to the
	 * given service id. Only messages with a timeout greater or equal to 0 are
	 * added.
	 * 
	 * @param message
	 *            the message to add.
	 ...
	 */
	protected void addMessageToServicePersistence(Message message,
			String serviceId) throws PersistenceException {
	...
	/**
	 * Returns the service persistence root directory. The path of the directory
	 * where persisted messages for a particular service id will be stored to
	 * and read from is constructed by concatenating the service id. For
	 * instance, if this directory points to "C:\messages\", messages associated
	 * to the service id "service-id1" will be read from
	 * "C:\messages\service-id1\".
	 */
	protected abstract File getServicePersistenceDirectory();

	/**
	 * Returns the persistence object that will be used to store to and read
	 * from messages associated to the specified service id.
	 * 
	 * @param serviceId
	 *            the service id.
	 * @return the persistence object that will be used to store to and read
	 *         from messages associated to the specified service id.
	 */
	protected Persistence getServicePersistence(String serviceId) {
		Persistence persistence = (Persistence) servicePersistences
				.get(serviceId);
		if (persistence == null) {
			servicePersistences.put(serviceId, persistence = new Persistence(
					new File(getServicePersistenceDirectory(), serviceId)));
		}
		return persistence;
	}
	 ...
	 * @param className
	 *            the class name.
	 * @return the registered class with the specified name, or
	...
	/**
	 * A Thread that process a messages list received through a SocketSession
	 * for a specified registered service.
	 * 
	 * @author aoviedo
	 * 
	 */
			...
			try {
				while (true) {
					...
					try {
						...
						logger.severe("Exception caught while processing the received message: "
								+ ex.getMessage());
					}
					synchronized (ServiceThread.class) {
						serviceThreadsExecuting--;
