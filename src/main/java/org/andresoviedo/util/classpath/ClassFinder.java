...
/**
 * <p>
 * This class implements the capability to search over the current classpath retrieving classes that implement a certain
 * interface.
 * </p>
 *
...
public class ClassFinder {
	 ...
	 * This would include all implementations of it, if it is an interface or it and all subclasses of it if it's a
	 * class.
	 * </p>
	 *
	 * @param superClass
	 *            the Class that should be searched for along with implementations and subclasses
	 */
	 ...
	 * and all subclasses of it if it's a class. The supplied <code>requiredPathSubstring must be part of the fully
	 * qualified classname.
	 * </p>
	 *
	 * @param superClass
	...
	/**
	 * <p>
	 * Used to retrieve the results <code>Set</code> from this harvester's search.
	 * </p>
	 *
			...
			for (int i = 0; i < children.length; i++) {
				...
				if (child.isDirectory()) {
					...
					if (child.getName().endsWith(".class")) {
						String className = getClassName(current + ((current == "") ? "" : File.separator)
								+ child.getName());
	...
	/**
	 * <P>
	 * Iterates through the files in a zip looking for files that may be classes. This is not recursive as zip's in
	 * zip's are not searched by the classloader either.
	 * </p>
	 *
	 * @param file
